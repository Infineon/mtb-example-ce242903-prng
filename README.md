# PSOC&trade; Control C1 MCU: PRNG

This code example demonstrates how to generate a random number sequence of length 10 using the pseudo-random number generation feature of the PSOC&trade; Control C1 MCU. The generated numbers are then displayed on a UART terminal emulator.

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce242903-prng)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[Prng](https://github.com/Infineon/mtb-example-ce242903-prng)" application with the desired name "Prng" configured for the *KIT_PSC1M3_EVK* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSC1M3_EVK --app-id mtb-example-ce242903-prng --user-app-name Prng --target-dir "C:/mtb_projects"
   ```
...
4. After programming, the application starts automatically. Confirm that "PSOC C1 MCU: PRNG: Pseudo Random Number Generation" is displayed on the UART terminal. Observe the generated random numbers
...
A pseudo random number (PRNG) is generated using a hardware random number generator that generates random numbers from a physical process. PRNG in the MCU generates pseudo random numbers of programmable bit size ranging from 0 bits to 16 bits.
...
| PRNG (PDL) | PRNG | Generates pseudo random numbers of programmable bit size |
...
Document title: *CE242903* – *PSOC&trade; Control C1 MCU: PRNG*
...
The Bluetooth&reg; word mark and logos are registered trademarks owned by Bluetooth&reg; SIG, Inc., and any use of such marks by Infineon is under license.
